...
func InitDriver(job *engine.Job) engine.Status {
	...
	if err != nil {
    // At this point we shoul have a bridge because of
    // IPv4. Throw an error
    job.Error(err)
    return engine.StatusErr
	} else {
...
func findBridgeNetwork(preferredIp string, inet6 bool) (string, error) {
	address_pool := []string{}
	nameservers := []string{}
...
func createBridge(bridgeIP, bridgeIP6 string) error {
	...
	inet, err := findBridgeNetwork(bridgeIP, false)
	if err != nil {
		return err
	}
	inet6, err = findBridgeNetwork(bridgeIP6, true)
	if err != nil {
		return err
	}
...
func Allocate(job *engine.Job) engine.Status {
	...
	// IPv6
	if requestedIP6 != nil {
		ip6, err = ipallocator.RequestIP6(bridgeNetwork6, &requestedIP)
	} else {
		ip6, err = ipallocator.RequestIP6(bridgeNetwork6, nil)
	}
	if err != nil {
		job.Error(err)
		return engine.StatusErr
	...
	out.Set("Mask6", bridgeNetwork.Mask.String())
	out.Set("Gateway", bridgeNetwork.IP.String())
	out.Set("Gateway6", bridgeNetwork.IP6.String())
	...
	// IPv6
	size, _ := bridgeNetwork6.Mask.Size()
	out.SetInt("IPPrefixLen6", size)
